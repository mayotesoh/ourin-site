...
## お問い合わせの設計
- **2026-08-29にフォームを廃止し、公式LINEのみに一本化**（`gas/Contact.gs` も削除。復活はGit履歴から）
- 窓口は2つ。`consts.ts` の `LINE_FORTUNE_URL`（占い）と `LINE_BUSINESS_URL`（テレアポ・営業代行）
- `/contact` は窓口カード2枚＋連絡の流れ＋注意書きのみの構成。各事業の詳細ページのCTAもLINEボタン1つ
